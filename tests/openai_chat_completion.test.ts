...
describe("Check chat completion unsupported requests", () => {
  ...
  test("Structural tag rejects a JSON-encoded nested schema", () => {
    ...
    expect(() =>
      ...
      "response_format.structural_tag.format.tags[0].content.json_schema must be a JSON Schema object or boolean",
    );
  });

  test("Structural tag accepts a nested schema object", () => {
    const request: ChatCompletionRequest = {
      messages: [{ role: "user", content: "Hello!" }],
      response_format: {
        type: "structural_tag",
